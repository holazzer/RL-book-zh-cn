...
之前我们引入了更新公式，还引入了误差的概念。假设说问题是动态的，也就是奖励是在变化的，那么我们应该更加重视最近的误差，比如，我们把公式改为
$$
Q_{n+1} = Q_n + \alpha[R_n - Q_n]
$$
让步长是一个常数$\alpha \in (0,1]$，这样展开就会发现，
$$
Q_{n+1} = （1-\alpha)^nQ_1+\sum_{i=1}^n\alpha(1-\alpha)^{n-i}R_i
$$

每个奖励的权重和时间久远程度有关系了，越老越不吃香。

干脆把$\alpha$写成一个序列${\alpha_n(a)}$，简单平均法就是$\alpha_n(a) = \frac{1}{n}$，步长法就是$\alpha_n(a) = Constant$.收敛的必要条件是：
$$
\sum_{n=1}^{\infin}\alpha_n(a) = \infin 和 \sum_{n=1}^{\infin}\alpha_n^2(a) < \infin
$$

第一个条件是为了保证$\alpha$不要太小，比随机波动还要小就没用了，第二个条件保证$\alpha$不要太大，太大可能导致Q发散。

$\alpha_n(a) = \frac{1}{n}$满足这两个条件，但是$\alpha_n(a)=\alpha$是不满足第二个条件的。所以实际上这样的方法收敛很慢，不好使。

**练习2.4** 请用$\alpha_n(a)$表示每一个奖励的权重。

**练习2.5** (编程) 自己编程试试，看看用简单平均法处理非静态问题的困难。修改一下试验台，让问题变成动态的，比如每次把方差加大一点点。然后也画一个图2.2那样的图，比较简单平均和步长$\alpha=0.1$。用$\epsilon=0.1，跑得长一点，比如一次跑10000步$。


## 2.6 乐观的初始值

咱们之前的所有方法都涉及到了一个事儿，就是赋初值。这个初值都是人为设定的，那就不是机器学到的。用统计学的话说，叫做**有偏的(biased)**。如果是简单平均的话，那等到每个动作都执行一次，所有的值都是计算得到的了；如果是增量更新的，那这个值就要一直缠着你。不好的就是这个值需要你来先选好，就算是都设成0也是选了。好处是，你可以用它来表示先验知识。

你也可以用初始值来激励探索。刚开始我们把初始值都设置为0，现在我们如果把它们都设为+5，那就不一样，因为获得的奖励是从$N(0,1)$里面产生的，所以+5显然是太大了，这样，不管是哪一个动作，它获得的奖励都显得小了，它就会“很失望”，所以下一次就不会去选这个行动，这样可以让收敛之前大家都被选到。

但是很显然这只能用于任务不变的情况，因为它只能影响最初的动作选择。
...
**练习2.6 神秘的刺**  图2.3的结果是2000次独立实验的平均值，应该是很可靠的，但是乐观初值法的开始出现了剧烈的摆动和尖刺，这是为什么？换句话说，为什么这个方法在刚一开始会表现得特别好？

**练习2.7 无偏的常数步长方法** 咱们一直用的都是简单平均方法，这样没有一开始的偏差，但是在非静态问题上简单平均的表现不好，能不能两者兼得呢？一种想法便是，用这样的步长：
$\beta_n = \alpha / \bar{o_n}$，$\alpha$是一个常数，$\bar{o_n}$是一个从0开始向1靠近的值：$\bar{o_n} =\bar{o_{n-1}} + \alpha(1-\bar{o_n}),\bar{o_n}=0$
分析证明这样更新的初值是没有初始偏差的。

...
强化学习是一种用于理解和自动化目标导向的学习与决策的计算学习方法。它与其他计算方法的区别在于，它强调由代理从与环境的直接交互中学习，而无需示例性的监督或完整的环境模型。我们认为，强化学习是第一个认真解决因与环境互动而学习以实现长期目标的过程中出现的计算问题的领域。


强化学习使用马尔可夫决策过程的形式框架来定义学习主体与学习者之间的互动在状态，行动和奖励方面的环境。该框架旨在成为表示人工智能问题的基本特征的简单方法。这些特征包括因果关系，不确定性和不确定性以及显性目标的存在。


价值和价值函数的概念是本书中大多数强化学习方法的关键。我们认为，价值功能对于策略空间中的有效搜索至关重要。价值函数的使用将强化学习方法与进化方法区别开来，后者是在整个策略评估的指导下直接在策略空间中进行搜索的。
...
最后，我也给大家准备了一些习题，这些问题就比较具体了。

1. 跟训练结果玩两把，你能赢吗？但是训练的时候，你可以看到，跟随机出招的玩家玩，会有10%的时候输掉。为什么？

2. 观察一下训练数据，里面还是有很多的0.5，为什么？

3. 由2，你能不能改写程序，让模型也可以玩O？（真的，如果你弄懂了2，这里真的超级简单，答案呼之欲出）